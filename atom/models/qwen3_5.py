...
def _layer_types_for_qwen35_text_config(config) -> list[str]:
    """Resolve per-layer attention kind for hybrid GDN stacks.

    HuggingFace ``Qwen3_5MoeTextConfig`` (e.g. transformers 5.x) may omit
    ``layer_types`` while ATOM's forked config always sets it. Mirror
    :class:`atom.model_config.qwen3_5_moe.Qwen3_5MoeTextConfig` defaults.
    """
...
class Qwen3_5Model(Qwen3NextModel):
    def __init__(self, *, atom_config, prefix: str = ""):
        ...
        layer_types = _layer_types_for_qwen35_text_config(config)
